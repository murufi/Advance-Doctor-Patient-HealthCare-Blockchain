...
contract HealthComplaints {
    ...
    function registerUser(address _user, string memory _role) public {
        // Add logic to determine who can register users (e.g., only admin)
        // For now, we'll allow anyone to register users
    ...
    function addComplaint(string memory _complaintText) public {
        require(checkRole(msg.sender, "patient"), "Only patients can add complaints");
    ...
    function replyToComplaint(uint _id, string memory _reply) public {
        require(checkRole(msg.sender, "doctor"), "Only doctors can reply to complaints");
        ...
        require(msg.sender != complaint.patient, "Doctor cannot reply to their own complaint");
